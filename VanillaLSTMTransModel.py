...
class VanillaLSTMTransModel():
    ...
    def __init__(self, args, training=True):
		...
		with tf.variable_scope("vanLSTM_decoder/decoder_accessory"):
			...
			output_converter_lambda = lambda cell_output_: tf.nn.softmax(logits=output_affine_map_lambda(cell_output_), dim=-1) # -1: last dimension
		...
		_, self.encoder_final_state = encoder.run(inputs=input_data_list,
		                                          chain_length=None,
		...
		self.cell_outputs, _ = decoder.run(inputs=input_data_list,
    ...
    def get_sum_of_cost(self, cell_outputs, targets, targets_lens):
		...
		affine_mapped_cell_outputs = [self.output_affine_map_lambda(cell_output_) for cell_output_ in cell_outputs]
